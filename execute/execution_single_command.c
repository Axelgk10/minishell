...
void	just_execute_it_man(t_shell *shell)
{
	...
	status = 0;
	path_env = ft_split(get_env_values(shell->env, "PATH"), ':');
	pid = fork();
	if (pid == -1)
	{
		int i = 0;
...
void	execute_builtin(t_shell *shell)
{
	...
	{
		shell->exit_status = change_directory(shell->commands->av[1], shell);
		...
		just_execute_it_man(shell); //Falta seguir revisando por aqui
}
